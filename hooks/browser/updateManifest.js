module.exports = function (context) {
  ...
  var platformProjPath = path.join(context.opts.projectRoot, "platforms/browser");
  ...
  fs.readFile(platformManifestJson, "utf8", function (err, platformJson) {
    ...
    var pluginManifestPath = path.join(
      ...
      "plugins/@equipapp/cordova-plugin-push/src/browser/manifest.json"
    );
    ...
    fs.readFile(pluginManifestPath, "utf8", function (err, pluginJson) {
      ...
      fs.writeFile(platformManifestJson, JSON.stringify(platformManifest), function (err) {
        if (err) {
